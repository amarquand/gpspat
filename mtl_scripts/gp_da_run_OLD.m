...
clsnum = [2,8];
cov = 'lin';
output_name = ['../gp_da/t2ml_',num2str(clsnum(1)),'_v_',num2str(clsnum(2)),'_YY0'];
opt.maxEval = 500;
...
ytrall = cell(1,Nfolds);
yteall = cell(1,Nfolds);
...
for f = 1:Nfolds
    ...
    %hyp = lh0;
    [hyp nlmls] = minimize(lh0, @gp_mtr, opt.maxEval, {Phi(tr,tr), Ystr}, {ytrall{f}}, opt);
