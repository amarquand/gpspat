...
for perm =1:opt.Nperms
    ...
    for f = 1:Nfolds
        ...
        if opt.computeWeights && ~bad
            ...
            for t = 1:T
                ...
                xid = find(ID(:,2) == f); % because only this subject is present
    ...
    if opt.saveResults & ~mod(perm,20)
