function [stats] = sp_linbf_gibbs_mh(X, Y, opt)
...
for g = 1:opt.nGibbsIter
    ...
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%   
    % conditional prior
    mur = (K(rid,~rid)/Kc)*f_new(~rid);
    Sr  = Kr - (K(rid,~rid)/Kc)*K(~rid,rid);
    
    % exact conditional posterior
    L_Sr     = chol(Sr + Noise(rid,rid))';       
    alpha    = solve_chol(L_Sr',y(rid)-mur);
    mur_post = mur + Sr*alpha;
    v        = L_Sr\(Sr');
    Sr_post  = Sr - v'*v;
